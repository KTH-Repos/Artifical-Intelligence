...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        # It is recommended you use the np.random module
        schedule_linear = ScheduleLinear(anneal_timesteps, epsilon_final)
        if np.random.rand() < schedule_linear.value(current_total_steps):
            # Explore: Choose a random action
            action = np.random.choice(all_actions)
        else:
            # Exploit: Choose the action with the highest Q-value for the current state
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                ...
                action = epsilon_greedy(Q, s_current, list_pos, current_total_steps, self.epsilon_initial, self.epsilon_final, eps_type="linear")
                # ADD YOUR CODE SNIPPET BETWEEN EX 5
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        return self.initial_p - fraction * (self.initial_p - self.final_p)
        # ADD YOUR CODE SNIPPET BETWEEN EX 4.2
